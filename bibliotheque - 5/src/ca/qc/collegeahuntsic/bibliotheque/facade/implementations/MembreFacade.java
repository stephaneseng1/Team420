...
public class MembreFacade extends Facade implements IMembreFacade {
	...
	MembreFacade(IMembreService membreService) throws InvalidServiceException {
		...
		if (membreService == null) {
			throw new InvalidServiceException(
					"Le service de membres ne peut être null");
		}
	 ...
	 */
	@Override
	public void inscrire(Session session, MembreDTO membreDTO)
			throws InvalidHibernateSessionException, InvalidDTOException,
			InvalidDTOClassException, FacadeException {
		try {
			getMembreService().inscrire(session, membreDTO);
