...
public class PretFacade extends Facade implements IPretFacade {
	...
	PretFacade(IPretService pretService) throws InvalidServiceException {
		...
		if (pretService == null) {
			throw new InvalidServiceException(
					"Le service de prêts ne peut être null");
		}
	...
	public void terminer(Session session, PretDTO pretDTO)
			...
			InvalidCriterionValueException {
		try {
			getPretService().terminer(session, pretDTO);
		} catch (ServiceException serviceException) {
			throw new FacadeException(serviceException);
		}
	}
