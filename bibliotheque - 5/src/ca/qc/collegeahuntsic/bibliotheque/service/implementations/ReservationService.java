...
public class ReservationService extends Service implements IReservationService {
	 ...
	 */
	ReservationService(IReservationDAO reservationDAO, IPretDAO pretDAO)
			throws InvalidDAOException {
		super();
	...
	public void placer(Session session, ReservationDTO reservationDTO)
		...
		if (aEteEmprunteParMembre) {
			throw new ExistingLoanException("Le livre " + unLivreDTO.getTitre()
					+ " (ID de livre : " + unLivreDTO.getIdLivre()
					+ ") est déjà prêté à " + unMembreDTO.getNom()
					+ " (ID de membre : " + unMembreDTO.getIdMembre() + ")");
		}
	...
	public void utiliser(Session session, ReservationDTO reservationDTO)
		...
		unMembreDTO.setNbPret(Integer.toString(Integer.parseInt(unMembreDTO
				.getNbPret()) + 1));
		getPretDAO().update(session, unMembreDTO);
