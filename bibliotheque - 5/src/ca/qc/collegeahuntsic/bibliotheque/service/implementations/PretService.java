...
public class PretService extends Service implements IPretService {
	...
	public PretDTO getPret(Session session, String idPret)
		...
		try {
			return (PretDTO) getPretDAO().get(session, idPret);
		} catch (DAOException daoException) {
			throw new ServiceException(daoException);
		}
	}
	 ...
	 */
	@Override
	public void updatePret(Session session, PretDTO pretDTO)
			throws InvalidHibernateSessionException, InvalidDTOException,
			InvalidDTOClassException, ServiceException {
		try {
			getPretDAO().update(session, pretDTO);
		} catch (DAOException daoException) {
			throw new ServiceException(daoException);
		}
	}
	 ...
	 */
	@Override
	public void deletePret(Session session, PretDTO pretDTO)
			throws InvalidHibernateSessionException, InvalidDTOException,
			InvalidDTOClassException, ServiceException {
		try {
			getPretDAO().delete(session, pretDTO);
		} catch (DAOException daoException) {
			throw new ServiceException(daoException);
		}
	}
	 ...
	 */
	@Override
	public List<PretDTO> getAllPrets(Session session, String sortByPropertyName)
	...
	public List<PretDTO> findByMembre(Session session, String idMembre,
		...
		try {
			return getPretDAO().findByMembre(session, idMembre,
					sortByPropertyName);
		} catch (DAOException daoException) {
			throw new ServiceException(daoException);
		}
	...
	public List<PretDTO> findByLivre(Session session, String idLivre,
			...
			ServiceException, InvalidCriterionValueException {
		try {
			return getPretDAO().findByLivre(session, idLivre,
					sortByPropertyName);
		} catch (DAOException daoException) {
			throw new ServiceException(daoException);
		}
	...
	public List<PretDTO> findByDatePret(Session session, Timestamp datePret,
			...
			ServiceException, InvalidCriterionValueException {
		try {
			return getPretDAO().findByDatePret(session, datePret,
					sortByPropertyName);
		} catch (DAOException daoException) {
			throw new ServiceException(daoException);
		}
	...
	public void commencer(Session session, PretDTO pretDTO)
		...
		PretDTO unPretDTO = getPret(session, pretDTO.getIdPret());

		MembreDTO unMembreDTO = unPretDTO.getMembreDTO();
		if (unMembreDTO == null) {
			throw new MissingDTOException("Le membre "
					+ pretDTO.getMembreDTO().getIdMembre() + " n'existe pas");
		}

		LivreDTO unLivreDTO = unPretDTO.getLivreDTO();
		if (unLivreDTO == null) {
			throw new MissingDTOException("Le livre "
					+ pretDTO.getLivreDTO().getIdLivre() + " n'existe pas");
		}
	...
	public void renouveler(Session session, PretDTO pretDTO)
		...
		PretDTO unPretDTO = getPret(session, pretDTO.getIdPret());
		if (unPretDTO == null) {
			throw new MissingDTOException("Le prêt " + pretDTO.getIdPret()
					+ " n'existe pas");
		}
		MembreDTO unMembreDTO = unPretDTO.getMembreDTO();
		if (unMembreDTO == null) {
			throw new MissingDTOException("Le membre "
					+ unPretDTO.getMembreDTO().getIdMembre() + " n'existe pas");
		}
		LivreDTO unLivreDTO = unPretDTO.getLivreDTO();
		if (unLivreDTO == null) {
			throw new MissingDTOException("Le livre "
					+ unPretDTO.getLivreDTO().getIdLivre() + " n'existe pas");
		}

		Set<PretDTO> prets = unLivreDTO.getPrets();
		if (prets.isEmpty()) {
			throw new MissingLoanException("Le livre " + unLivreDTO.getTitre()
					+ " (ID de livre : " + unLivreDTO.getIdLivre()
					+ ") n'est pas encore prêté");
		}
		boolean aEteEmprunteParMembre = false;
		for (PretDTO unAutrePretDTO : prets) {
			aEteEmprunteParMembre = unMembreDTO.equals(unAutrePretDTO
					.getMembreDTO());
		}
		if (!aEteEmprunteParMembre) {
			throw new ExistingLoanException("Le livre " + unLivreDTO.getTitre()
					+ " (ID de livre : " + unLivreDTO.getIdLivre()
					+ ") n'a pas été prêté à " + unMembreDTO.getNom()
	...
	public void terminer(Session session, PretDTO pretDTO)
		...
		try {
			...
			List<PretDTO> prets = findByMembre(session,
					unMembreDTO.getIdMembre(), PretDTO.DATE_PRET_COLUMN_NAME);
			if (prets.isEmpty()) {
			...
			if (!aEteEmprunteParMembre) {
				throw new ExistingLoanException("Le livre "
						+ unLivreDTO.getTitre() + " (ID de livre : "
						+ unLivreDTO.getIdLivre() + ") n'a pas été prêté à "
						+ unMembreDTO.getNom() + " (ID de membre : "
						+ unMembreDTO.getIdMembre() + ")");
			}
			unMembreDTO.setNbPret(Integer.toString(Integer.parseInt(unMembreDTO
