...
CREATE TABLE membre (idMembre   NUMBER       CHECK (idMembre > 0),
...
CREATE TABLE livre (idLivre         NUMBER       CHECK (idLivre > 0),
                    titre           VARCHAR(100) NOT NULL,
                    auteur          VARCHAR(100) NOT NULL,
                    dateAcquisition TIMESTAMP    NOT NULL,
                    CONSTRAINT      cleLivre     PRIMARY KEY (idLivre));

CREATE TABLE pret (idPret     NUMBER     CHECK (idPret > 0),
...
CREATE TABLE reservation (idReservation   NUMBER     CHECK (idReservation > 0),
                          idMembre        NUMBER     CHECK (idMembre > 0),
                          idLivre         NUMBER     CHECK (idLivre > 0),
